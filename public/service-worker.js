...
// 1. CONFIG
// ───────────────────────────────────────────────────────────────────────────────────────
const CACHE_NAME = 'finance-manager-v14';  // ← Bump version every deploy!
...
self.addEventListener('fetch', event => {
  ...
  if (request.method !== 'GET' || url.pathname.includes('/api/') || url.origin !==
      location.origin || url.href.includes('chrome-extension://') ||
      url.href.includes('firebase') || url.href.includes('google-analytics')) {
    return;
  }
  ...
  event.respondWith(caches.match(request).then(cachedResponse => {
    ...
    return fetch(request).then(networkResponse => {
      if (!networkResponse || networkResponse.status !== 200 || networkResponse.type !==
          'basic') {
...
self.addEventListener('push', event => {
  const data = event.data?.json() ||
      { title: 'FinanceFlow', body: 'You have a new notification' };

  event.waitUntil(self.registration.showNotification(data.title, {
    body: data.body, // icon: `${base}icons/icon-192x192.png`,
    // badge: `${base}icons/icon-72x72.png`,
    tag: 'finance-notification', data: { url: data.url || './' },
  }));
});
