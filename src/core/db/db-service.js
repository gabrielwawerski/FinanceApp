...
function makeToken() {
  return crypto.randomUUID?.() || 'local-' + Math.random().toString(36).slice(2) + Date.now().toString(36);
}
...
export const DatabaseService = {
  ...
  async verifyLogin(usernameOrEmail, password) {
    try {
      ...
      const encoder = new TextEncoder();
      const saltBytes = Uint8Array.from(user.passwordSalt.match(/.{2}/g).map(b => parseInt(b, 16)));
      const key = await crypto.subtle.importKey('raw', encoder.encode(password), 'PBKDF2', false, ['deriveBits']);
      const bits = await crypto.subtle.deriveBits(
         {name: 'PBKDF2', salt: saltBytes, iterations: 600000, hash: 'SHA-256'},
         key,
      ...
      const attemptHash = Array.from(new Uint8Array(bits)).map(b => b.toString(16).padStart(2, '0')).join('');
  ...
  async getTransactions(userId, startDate = null, endDate = null) {
    try {
      ...
      if (startDate || endDate) {
        ...
        const e = endDate ? new Date(endDate.setHours(23, 59, 59, 999)) : new Date(8640000000000000);

        collection = collection
           .between([userId, s], [userId, e], true, true);
      } else {
